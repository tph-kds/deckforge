...
- **Export fidelity (informational)** — CI's `export-goldens` job builds and serves `examples/02-example`, captures a deterministic screenshot of every slide at native canvas size via Playwright, and compares the PNG dimensions and the JSON manifest structure against the committed goldens in `tests/goldens/` with `python scripts/validate/compare_export_goldens.py`. The job is informational and **not release-blocking** because browser rendering varies across machines. **PPTX round-trip is out of scope for CI**: byte-level PPTX fidelity requires external PowerPoint/OpenXML extraction tooling and is intentionally not gated here; PNG/JSON export fidelity is the automated contract.
