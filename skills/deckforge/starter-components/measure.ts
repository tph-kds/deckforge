...
import { blocksBySlot, resolveSlotFrame, type LayoutContract } from './layout-engine';
...
export type MeasureIssueCode = 'overflow' | 'collision' | 'budget' | 'boundary' | 'orphan';

export interface MeasureIssue {
  severity: 'warning' | 'error';
  code: MeasureIssueCode;
  slot: string;
  blockId?: string;
  message: string;
...
function textOf(block: DeckBlock): string {
  ...
  if (content && typeof content === 'object') {
    ...
    if (Array.isArray(items)) return items.filter((x): x is string => typeof x === 'string').join(' ');
 ...
 */
export function measureBlockOverflow(block: DeckBlock, frame: Frame): MeasureIssue | undefined {
 ...
 */
export function detectCollisions(deck: DeckProject, slide: DeckSlide, contract: LayoutContract): MeasureIssue[] {
  const issues: MeasureIssue[] = [];
  const bySlot = blocksBySlot(slide);
  const regions: Array<{ label: string; frame: Frame; intentional: boolean; blockId?: string }> = [];
  for (const slot of contract.slots) {
    const blocks = bySlot.get(slot.id) ?? [];
    const frame = resolveSlotFrame(deck, slot);
    for (const block of blocks) {
      regions.push({ label: `slot:${slot.id}`, frame, intentional: false, blockId: block.id });
    }
  }
 ...
 */
export function measureSlide(deck: DeckProject, slide: DeckSlide, contract: LayoutContract): MeasureIssue[] {
  const issues: MeasureIssue[] = [];
  const slotById = new Map(contract.slots.map((slot) => [slot.id, slot]));
  const frameById = new Map(contract.slots.map((slot) => [slot.id, resolveSlotFrame(deck, slot)]));
  ...
  for (const binding of slide.layoutBindings ?? []) {
    ...
    const budgetChars = (slot as { contentBudget?: { maxCharacters?: number } }).contentBudget?.maxCharacters;
    if (budgetChars != null) {
  ...
  issues.push(...detectCollisions(deck, slide, contract));

  const order: Record<MeasureIssueCode, number> = { overflow: 1, collision: 2, budget: 3, boundary: 4, orphan: 5 };
